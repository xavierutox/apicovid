...
def vacunas():
    ...
    if(tipo=="Fallecidos"):
        ...
        for i in range(len(df)):
            datos={}
            datos["semana"]=df_list[i][0]
            datos["sin_vacunar"]=df_list[i][8]
            datos["1_dosis"]=df_list[i][9]
            datos["2_dosis"]=df_list[i][10]
            datos["completo"]=df_list[i][11]
            datos["dosis_unica"]=df_list[i][12]
            datos["dosis_unica_completo"]=df_list[i][13]
            datos["refuerzo"]=df_list[i][14]
            casos.append(datos)
        return{
            "casos":casos
        }
